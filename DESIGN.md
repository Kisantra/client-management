...
### The Mark
- **The mark joins the palette; the brand's sian does not.** The Kisantra monogram as published on Instagram and TikTok carries a sian arm, `#42b1cc`. In this app that arm is recoloured to Teal Ink (`{colors.primary-deep}`) on a Deep Teal tile: the sian is a different hue from the teal voice, and a second hue in the rail's top-left would read as a third meaning beside "here" and "breach". The sian appears nowhere in the product. The arm is quiet on purpose — 1.6:1 against its own tile, a recessed notch rather than an accent.
- **The tile is pinned.** `{colors.brand-tile}` is Deep Teal in *both* themes and does not follow `{colors.primary}`, which becomes the data-only Bright Teal under `.dark` — a white mark on that clears 2.4:1. Pinned, the tile holds 3.4:1 against the dark rail and 5.2:1 under its own mark. It is the one colour in the system that ignores the theme, because a mark that changes weight when the theme flips stops being one mark.
- **Plain below 100px.** The mark's batik fill repeats every ~40px at source size, so under roughly 100px it stops reading as a pattern and flattens to grey, taking the contrast with it. Everything the app draws — the rail's 32px tile, the auth mark, the favicon — ships the plain rendition (`public/img/kisantra-mark.png`, white body and Teal Ink arm on a transparent ground, the tile drawn in CSS). The batik survives on the 180px touch icon alone, where its lines take the tile's own teal and read as holes cut through the body.
